...
def measureHeithByStepSize(nozzle, hightActuator, zStartTest, stepsize, position):
    ...
    for pos in range(0,2050): # TODO 2000
...
def measure_height(nozzle, position):
    ...
    valType = hightActuator.getValueType()
    print valType
    hightActuator.actuateProfile("String")
    ...
    # measure with 0.05mm precistion
    heightOfMeasurement = measureHeithByStepSize(nozzle, hightActuator, zStartTest, 0.05, position)

    testValueWriter = csv.writer(open(python_scripts_folder+'messdata.csv', 'a'), delimiter=' ') #, quotechar='', quoting=csv.QUOTE_MINIMAL)
    testValueWriter.writerow(['height', heightOfMeasurement])
